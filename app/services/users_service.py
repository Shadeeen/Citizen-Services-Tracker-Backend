...
async def update_user(user_id: str, patch: dict):
    ...
    update_doc = {}
    if "full_name" in patch: update_doc["full_name"] = patch["full_name"]
    if "role" in patch: update_doc["role"] = patch["role"]
    if "is_active" in patch: update_doc["is_active"] = patch["is_active"]
    if "password" in patch: update_doc["password_hash"] = hash_password(patch["password"])
